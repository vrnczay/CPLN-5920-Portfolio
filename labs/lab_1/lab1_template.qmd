...
**Pattern Analysis:** Since almost all the tracts have issue with high MOE in their ethnic groups, I only analyzed the data quality problem within the tracts that with MOE issues because there are only 1 tracts without MOE issues and this is not representative (and that tract is showing NA data so thats a data glitch or no-data-tract, which is not helpful for the analysis.)
